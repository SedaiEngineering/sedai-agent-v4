...
-   **Simplified Server Authentication**: The `--auth` and `--authfile` flags have been removed from the server. Authentication is now handled exclusively by passing a JSON string via a command-line flag (`--auth-json`) or an environment variable (`CHISEL_AUTH_JSON`).
